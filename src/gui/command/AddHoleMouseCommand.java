 ...
 * @since October 2005
 */
public class AddHoleMouseCommand implements Command, Cloneable {
...
//*****************************************************************************
    ...
    /** Constructor method. */
    public AddHoleMouseCommand(JFrame parent) {
    ...
    public void execute() {
        ...
        if (ini == null){
            if (controller.contains(po)) {
				...
				parent.addOutputText("The entire hole must be inside the boundary.");
			}
        }else{
			if (controller.contains(po)) {
				if (po.getX()!=ini.getX() || po.getY()!=ini.getY()) {
					...
					((SectionController)controller).getModel().getGModel().addHole(gp);
					controller.compose();
					ini = fin;
					PreviewLineCommand pl = new PreviewLineCommand(parent);
					pl.setInitialPoint(da.getViewPoint(ini));
...
//*****************************************************************************
    ...
    public Object clone() {
        try {
            AddHoleMouseCommand cmd = (AddHoleMouseCommand)super.clone();
			return cmd;
